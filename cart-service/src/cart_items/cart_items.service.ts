...
export class CartItemsService {
  ...
  async create(payload: CreateCartItemDto) {
    const [foundFood, foundUser, foundCartItem] = await Promise.all([
      ...
      await lastValueFrom(
        this.userService
          .send('findOneUser', payload.user_id)
  ...
  async getItemUser(user_id: number) {
    ...
    const cartItem: cart_items[] = await this.prismaService.cart_items.findMany(
      {
        where: {
          cart_id: user_id,
        },
    ...
    const foodIds = cartItem.map((item: cart_items) => item.food_id);
    if (foodIds.length > 0) {
